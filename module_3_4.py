...
содержит одно из этих слов. После вернуть список same_words в качестве
результата своей работы.

'''
def single_root_words (root_word, *other_words):
    same_words = []
    root_word = root_word.lower()          # унифицируем регистр символов для сравнения строк,
    for i in other_words:                   # перебераем предполагаемо подходящие слова.
        other_words = i.lower()             # унифицируем регистр символов для сравнения строк
        if root_word in other_words or other_words in root_word: # условие для слов списка other_words, которые содержат root_word или наоборот root_word содержит одно из этих слов.
           same_words.append(i)            # добавляем в список

    return same_words                       # возвращаем
